...
function Courses() {
  ...
  // Gọi API lấy danh sách khóa học và danh mục động từ Backend
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // 2. Lấy danh sách danh mục từ API Admin/Categories
        const catResponse = await axiosClient.get("/api/admin/categories");
        const catData = Array.isArray(catResponse.data)
          ? catResponse.data
          : catResponse.data.categories || [];

        // Chuyển đổi dữ liệu trả về thành định dạng cho các nút lọc
        const formattedCategories = [
          { label: "Tất cả khóa học", value: "All" },
          ...catData.map((cat) => ({
            label: cat.name,
            value: cat.name, // Khớp với trường category của khóa học trong CSDL
          })),
        ];
    ...
    <div className="courses-page bg-[#F8FAFC] min-h-screen">
      ...
      <section className="courses-content py-12 lg:py-16">
        <div className="courses-content__container mx-auto max-w-7xl px-6">
          {/* Categories Filter Tabs (Lấy động từ API Database) */}
          ...
          {loading && (
            <div className="text-center py-16 text-slate-500 font-medium">
              Đang tải danh sách khóa học và danh mục từ máy chủ...
            </div>
          )}
                  ...
                  >
                    <div>
                      ...
                      <div className="h-48 w-full rounded-2xl bg-gradient-to-tr from-slate-100 to-slate-200 mb-6 flex flex-col justify-between p-4 relative overflow-hidden">
                        ...
                        <span className="self-start bg-white/90 backdrop-blur-md text-emerald-600 font-extrabold text-[11px] px-3 py-1 rounded-full shadow-sm z-10">
                          ✓ {course.commitment || "Cam kết chất lượng"}
                        </span>
                      </div>
                      ...
                      <div className="mt-4 flex items-center gap-3 pt-4 border-t border-slate-100">
                        ...
                        <div>
                          ...
                          <p className="text-[11px] font-medium text-slate-400">
                            {course.instructor?.role || "Chuyên gia đào tạo"}
                          </p>
                        </div>
                      </div>
