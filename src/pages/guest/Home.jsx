...
function Home() {
  ...
  // Gọi API lấy danh sách khóa học thật từ Backend/MySQL khi trang được tải
  useEffect(() => {
    const fetchFeaturedCourses = async () => {
      try {
        ...
        // Lấy 3 khóa học đầu tiên làm khóa học nổi bật
        setFeaturedCourses(coursesArray.slice(0, 3));
      } catch (err) {
        console.error("Lỗi khi tải khóa học nổi bật:", err);
        setError("Không thể tải danh sách khóa học từ máy chủ.");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <>
      ...
      {/* Featured Courses (Lấy dữ liệu thật từ MySQL thông qua API) */}
                  ...
                  >
                    <div>
                      <div className="home-courses__image h-52 w-full rounded-2xl bg-gradient-to-br from-blue-900 via-indigo-950 to-slate-900 overflow-hidden relative shadow-inner p-5 flex flex-col justify-between">
                        ...
                        <span className="self-start rounded-full bg-white/25 backdrop-blur-md px-3.5 py-1 text-[11px] font-bold text-white border border-white/20 shadow-sm z-10">
                          {course.level || "Cơ bản"}
                        </span>
                        <div className="z-10">
                      ...
                      <div className="home-courses__info mt-6 flex items-center justify-between text-xs font-bold text-slate-500">
                        <span className="bg-slate-100 px-3 py-1.5 rounded-xl">
                          📚 {course.lessons_count || course.lessons || 0} bài
                          học
                        </span>
                      ...
                      <p className="home-courses__students mt-4 text-xs font-semibold text-slate-500">
                        👥 {course.students_count || course.students || 0} học
                        viên đang tham gia học tập
                      </p>
                    </div>
                    ...
                    <div className="mt-8 pt-4 border-t border-slate-100 flex items-center justify-between">
                      ...
                      <Link
                        ...
                        className="home-courses__btn inline-flex items-center justify-center rounded-xl bg-slate-900 px-4 py-2.5 text-xs font-black uppercase tracking-wider text-white transition-all duration-300 hover:bg-blue-600 shadow-md shadow-slate-900/20 active:scale-95"
                      >
                        Xem chi tiết &rarr;
                      </Link>
                    </div>
